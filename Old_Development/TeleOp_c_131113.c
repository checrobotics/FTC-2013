...
#pragma config(Servo,  srvo_S1_C1_1,    RightFlipper,         tServoNone)
#pragma config(Servo,  srvo_S1_C1_2,    servo2,               tServoNone)
#pragma config(Servo,  srvo_S1_C1_3,    servo3,               tServoNone)
#pragma config(Servo,  srvo_S1_C1_4,    LeftFlipper,          tServoNone)
...
task main()
{
  while(true)                            // Infinite loop:
  {
    ...
    {
      servo[servo1] = 184;       // Lower Servo 1 to position 20.
    }


  if(joy1Btn(6))          // If Button  6 on controller 1 is pressed:
    {
      servo[RightFlipper] = 71;      // Raise the rigt flipper to the opean position 130.
    }

    if(joy1Btn(8))          // If Button 8 on controller 1 is pressed:
    {
      servo[RightFlipper] = 255;       // Lower Servo 1 to position 20.
	  }
  ...
  nMotorEncoderTarget[bucket] = 400;  // target 240 degrees to dump the bucket - temporarily changed from 960 to 400 counts for testing on s/w robot
 ...
 if(joy1Btn(2)&& SensorValue(touchBucketZero)== 0)      // If the button 2 on controller 1 is pressed & touch sensor is not pressed :
  {
  // nMotorEncoder[bucket] = 0;
  // nMotorEncoderTarget[bucket] = 960;  // target 240 degrees to dump the bucket
  while (SensorValue(touchBucketZero) != 1)							// bucket motor will move clockwise until touch sensor is pressed
  {
  	motor[bucket] = 50;
  //while(nMotorRunState[bucket] != runStateIdle)  // While Motor B is still running:
	//{
  // Do not continue.
	//}
  }
