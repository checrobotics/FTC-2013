...
task main()
{
  while(true)                            // Infinite loop:
  {
    ...
    bucketDrive = 1500;					// define bucket drive position
    ...
    motor[Left_drive] = joystick.joy2_y2;         // Motor D is assigned a power level equal to the right analog stick's Y-axis reading.
    motor[Right_drive] = joystick.joy2_y1;         // Motor E is assigned a power level equal to the left analog stick's Y-axis reading.
  ...
  nMotorEncoderTarget[bucket] = 1500;  // target drive position
  motor[bucket] = -25;
 ...
 if(joy1Btn(3))      // If the button 3 on controller 1 is pressed, bucket moves to position to dump blocks and returns to bucket drive postion
  ...
  nMotorEncoderTarget[bucket] = 1500;  // target bucket dumping position
  motor[bucket] = -25;
  ...
  // {
  nMotorEncoderTarget[bucket] = 1500;  // target bucket drive position
  motor[bucket] = 25;
  while(nMotorRunState[bucket] != runStateIdle)  // While Motor is still running, allow to go to target posititon
		{
  	// Do not continue.
