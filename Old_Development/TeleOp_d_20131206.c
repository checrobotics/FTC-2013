...
task main()
{
  while(true)                            // Infinite loop:
  {
    ...
    bucketDrive = 200;					// define bucket drive position
    bucketDump = 1440;					// define bucket dump position to be 120 degrees from down position; 3:1 gear ratio --> 360 degrees of rotation*1440 counts/revolution
    L_Flip_Open = 200;
    L_Flip_Close = 25;
   	R_Flip_Open = 42;
    R_Flip_Close = 225;
  ...
  nMotorEncoderTarget[bucket] = bucketDrive;  // target drive position
  motor[bucket] = -25;
 ...
 if(joy1Btn(3) & SensorValue(touchBucketZero)== 1)      // If the button 4 on controller 1 is pressed and the bucket is in the down position, bucket moves to position to dump blocks
  {
  nMotorEncoder[bucket] = 0;
