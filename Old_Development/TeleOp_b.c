...
task main()
{
  ...
  if(joy1Btn(6))          // If Button  6 on controller 1 is pressed:
    {
      servo[Right_Flipper] = 71;      // Raise the rigt flipper to the opean position 130.
    }

    if(joy1Btn(8))          // If Button 8 on controller 1 is pressed:
    {
      servo[Right_Flipper] = 255;       // Lower Servo 1 to position 20.
	  }
  ...
  if(joy1Btn(2))      // If the button 4 on controller 1 is pressed
  {
    motor[bucket] = 50;
    wait1Msec(200); // Bucket runs at a power level of 50.
    motor[bucket] = 0;
    wait1Msec(20);
  }

  if(joy1Btn(4))      // If the button 2 on controller 1 is pressed :
  {
    motor[bucket] = -50;
    wait1Msec(200);// Bucket runs at the opposite direction.
    motor[bucket] = 0;
    wait1Msec(20);
  }
