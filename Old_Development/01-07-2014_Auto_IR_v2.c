...
#include "C:\Program Files (x86)\Robomatter Inc\ROBOTC Development Environment\Sample Programs\NXT\3rd Party Sensor Drivers\drivers\hitechnic-irseeker-v2.h"  // IR Seeker Driver
...
void driveBackward(int power, int time) {
					motor[Left_drive] = -1 * power;
				  motor[Right_drive] = -1 * power;
				  wait1Msec(time);
				  motor[Left_drive] = 0;
				  motor[Right_drive] = 0;
...
void rotateClockwise(int power, int time) {
...
void rotateCounterClockwise(int power, int time) {
...
task main()
{
    ...
    int bucketDump = 		1600;
    int bucketStop = 		800;					// stop the bucket MIDWAY //
    int L_Flip_Open = 	15;
